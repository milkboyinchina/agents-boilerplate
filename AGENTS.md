...
1. **Folder naming**: spaces become `_`, always append `_protocol` (e.g. `tier_routing_protocol/`, `question_protocol/`). Never kebab-case, never bare names. All runtime state lives under `.protocol/` (e.g. `.protocol/green_amber_red_workspace/`, `.protocol/redteam/`) — unmistakably distinct from source folders by construction; one-letter differences are a defect.
2. **Listing order**: protocol-listing tables and sections are alphabetical by protocol folder name (green, handoff, question, tier). The green 12-command table keeps its own owner→alphabetical rule — different table, different sort key.
...
4. **Runtime gitignore**: each bootstrap CLI ensures the single `.protocol/` line at init (idempotent, skip-if-present, stale per-path lines pruned); source folders stay committed. Users opt out by deleting the line.
