...
* **Dumb cron (~zero tokens):** a plain script follows each tool's `discover: {models_file: <path>}` hint, hashes the live ID lists, and overwrites exactly one file: `.protocol/tier_routing_workspace/heartbeat.json` (gitignored). Exit `0` written / `2` drift vs previous heartbeat / `1` all tools failed.
...
* `.protocol/tier_routing_workspace/heartbeat.json` — weekly cron output.
* `.protocol/tier_routing_workspace/routing_state.json` — pins + resolver state.
